...
如果希望每次启动 Codex 后自动使用主题，可以在首次安装完成后继续说：

> 帮我将这套 Diana Codex 主题设置为启动后自动生效。

Codex 应优先保存原生外观配置，或使用当前版本正式支持、可随时撤销的主题／插件持久化入口。完整角落美术只有在存在这类安全入口时才可以自动加载；没有正式入口时，自动生效仅包含能够由 Codex 自身保存的配色与字体，立绘和装饰素材继续保留在主题包中。

> [!WARNING]
> “启动后自动生效”不得通过 CDP、远程调试端口、后台 watcher、HTTP／WebSocket 监听、计划任务、登录启动项或强制重启 Codex 实现，也不得修改 `app.asar`、应用签名或系统安装文件。Codex 无法在这些边界内完成完整挂载时，应明确停止并保留素材，不能退回旧版注入方案。
