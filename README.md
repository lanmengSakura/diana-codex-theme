...
> 请使用 $skill-installer 从 https://github.com/lanmengSakura/diana-codex-theme/tree/main/skills/diana-codex-theme 安装 Diana Codex Theme。安装完成后只告诉我安装路径和验证结果，本条消息不要继续挂载主题。

安装器会把 Skill 放进当前用户的 Codex Skills 目录，新 Skill 从**下一条消息**开始可用。安装完成后另发一条：

> 使用 $diana-codex-theme 帮我完整挂载这套 Diana Codex 主题。

第二条指令默认要求完整效果，包括角落立绘、阿草、星星、糖果和叙事线稿。Codex 会优先使用正式主题、插件、Pet 或用户样式入口；若当前版本只能通过实验性 CDP 完成，它必须先通知用户同机访问风险以及准备创建的端口、进程、文件、日志和恢复方法，等待确认后再继续。

公开包不会下载或恢复旧 watcher，也不会把适配器写进 Codex 安装包。实验适配器由用户自己的 Codex 根据当前系统和 Codex 版本生成在用户可写目录中。用户不接受 CDP 风险时，才降级为原生配色并保留完整美术素材。

### Release ZIP 备用方式

最新版 Release 仍提供 `diana-codex-theme-skill-0.2.4.zip`，用于离线保存或手动安装。ZIP 是 Skill 分发包，**拖进对话并不等于已经注册到 Codex**。把 ZIP 附到任务后，应明确告诉 Codex：

> 请把附件中的 diana-codex-theme Skill 安装到我的 Codex Skills 目录，验证 SKILL.md 后只报告安装结果；下一条消息我再调用它。

仍需在下一条消息中使用 `$diana-codex-theme` 完成主题部署。若只是解压、复制素材或打开预览页，均不能算作主题已经挂载。

### 如何判断是否成功

- **Skill 安装成功**：用户 Skills 目录中存在 `diana-codex-theme/SKILL.md`，且下一条消息能识别 `$diana-codex-theme`。
- **配色部署成功**：真实 Codex 的日间或暗夜原生配色已经变化，并完成恢复验证。
- **完整挂载成功**：真实 Codex 工作区能看到立绘与装饰，且点击、输入、滚动、主题切换和恢复均已实测。
- 只复制文件、生成方案、通过 ZIP 校验或打开模拟页面，都不等于真实主题已经挂载。
...
`v0.2.4` 修正了 `v0.2.3` 把“安装 Skill”和“挂载主题”挤在同一条消息里的流程问题：

- 推荐安装改为 `$skill-installer` + GitHub Skill 路径，不再把拖入 ZIP 描述成确定性的自动安装入口。
- 安装与挂载拆成两条消息，确保新 Skill 已被 Codex 加载后再调用。
- Release ZIP 保留为离线和手动安装备用，并明确要求安装到用户 Skills 目录。
- Skill 必须分别报告“Skill 已安装”“原生配色已部署”“完整美术已挂载”，不得把复制文件或生成方案误报为完成。
- 完整美术仍以真实 Codex 界面为验收对象；预览页、CSS 校验和 ZIP 校验只能证明素材完整。
...
从最新版 Release 下载 `diana-terminal-0.2.4.zip`，解压后任选一种方式：
