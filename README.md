...
Codex 自身保存的配色与字体会在下次启动时继续生效，不需要额外程序。完整角落立绘目前没有官方自动加载接口；如果更看重启动后的完整视觉效果，也可以让自己的 Codex制作本地自动挂载方案：
...
> [!CAUTION]
> v0.2.2 公开发行包不包含监听器和自动注入脚本。自行开启本地监听属于用户主动选择的实验方案，不是本项目默认安装方式；不接受上述风险时，保留 Codex 原生配色即可。
