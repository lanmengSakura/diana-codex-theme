...
- Chinese narration source: `video/narration-zh-CN.txt`
- Timed subtitle source: `video/subtitles-zh-CN.srt`
- Original background-music generator: `video/generate-bgm.mjs`
- The archived v2 demo uses an original deterministic bell, pluck, pad and soft-bass arrangement generated by the repository script. It does not sample, reproduce, remix or redistribute an official A-SOUL song, a Diana cover, or a third-party recording.
- The narration is generated from the repository script text; no Diana voice sample or cloned character voice is used.
- The v4 non-commercial fan-demo mix uses a short instrumental excerpt from the published official `Hopeful Dreamer` MV. The source, credits and handling boundary are recorded in `video/hopeful-dreamer-source.md`. No official source recording, full song, isolated stem or standalone accompaniment is committed to this repository or bundled with the theme packages.

The music generator and narration/subtitle text are covered by the repository's MIT License. Any external song recording is not covered by the MIT License. Rendered videos still contain the Diana visual assets described above and therefore remain non-commercial fan works rather than all-assets-MIT packages.
